...
@section('title', 'Edit Manajemen Jasa')
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card card-primary">
                        <form action="{{ route('admin.services.services.update', $service->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="card-body">
                                <div class="row">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="number" class="form-control @error('base_price') is-invalid @enderror" 
                                                       id="base_price" name="base_price" value="{{ old('base_price', $service->base_price) }}" min="0" required>
                                                @error('base_price')
                                                    <span class="invalid-feedback" role="alert">
                                                        <strong>{{ $message }}</strong>
                                                    </span>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control @error('price_unit') is-invalid @enderror" 
                                                ...
                                                @foreach(\App\Models\Service::PRICE_UNITS as $key => $label)
                                                    <option value="{{ $key }}" 
                                                        {{ old('price_unit', $service->price_unit) == $key ? 'selected' : '' }}>
                                                        {{ $label }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            ...
                                            <select class="form-control @error('owner_id') is-invalid @enderror" 
                                                ...
                                                @foreach($owners as $owner)
                                                    <option value="{{ $owner->id }}" 
                                                        {{ old('owner_id', $service->owner_id) == $owner->id ? 'selected' : '' }}>
                                                        {{ $owner->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="number" class="form-control" id="display_order" 
                                                   name="display_order" value="{{ old('display_order', $service->display_order) }}" min="0">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <small class="form-text text-muted">
                                        Kosongkan jika tidak ingin mengubah gambar. Format: JPG, JPEG, PNG (Maksimal 2MB)
                                    </small>
